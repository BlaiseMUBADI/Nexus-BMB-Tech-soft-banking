...
-- =====================================================================
-- PREREQUIS - INDEXES POUR TABLES REFERENCEES (legacy-safe)
-- Evite les erreurs FK type #1822 sur des schemas existants.
-- =====================================================================
...
SET @sql := (
  SELECT IF(
    EXISTS (
      ...
      WHERE table_schema = @db AND table_name = 'tb_devises' AND ENGINE <> 'InnoDB'
    ),
    'ALTER TABLE tb_devises ENGINE=InnoDB',
    'SELECT "Engine tb_devises OK (InnoDB ou table absente)" AS info'
  )
);
...
SET @sql := (
  SELECT IF(
    EXISTS (
      ...
      WHERE table_schema = @db AND table_name = 'tb_devises' AND column_name = 'code_iso'
    )
    AND NOT EXISTS (
      SELECT 1 FROM information_schema.statistics
      WHERE table_schema = @db AND table_name = 'tb_devises' AND column_name = 'code_iso' AND seq_in_index = 1
    ),
    'ALTER TABLE tb_devises ADD INDEX idx_tb_devises_code_iso (code_iso)',
    'SELECT "Index sur tb_devises.code_iso deja present (ou table/colonne absente)" AS info'
  )
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql := (
  SELECT IF(
    ...
    'ALTER TABLE tb_agents ADD INDEX idx_tb_agents_matricule (matricule)',
...
SET @sql := (
  SELECT IF(
    EXISTS (
      SELECT 1 FROM information_schema.tables
      WHERE table_schema = @db AND table_name = 'tb_plan_comptable' AND ENGINE <> 'InnoDB'
    ),
    'ALTER TABLE tb_plan_comptable ENGINE=InnoDB',
    'SELECT "Engine tb_plan_comptable OK (InnoDB ou table absente)" AS info'
  )
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql := (
  SELECT IF(
    EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = @db AND table_name = 'tb_plan_comptable' AND column_name = 'numero_compte'
    )
    AND NOT EXISTS (
      SELECT 1 FROM information_schema.statistics
      WHERE table_schema = @db AND table_name = 'tb_plan_comptable' AND column_name = 'numero_compte' AND seq_in_index = 1
    ),
    'ALTER TABLE tb_plan_comptable ADD INDEX idx_tb_plan_comptable_numero_compte (numero_compte)',
    'SELECT "Index sur tb_plan_comptable.numero_compte deja present (ou table/colonne absente)" AS info'
  )
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- =====================================================================
-- 210000 - tb_commission_rules + tb_transaction_commissions + colonne
-- =====================================================================
...
SET @sql := (
  ...
  WHERE table_schema = @db
    ...
    AND constraint_name = 'tb_comm_rules_devise_fk'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql := (
  ...
  WHERE table_schema = @db
    AND table_name = 'tb_commission_rules'
    AND constraint_name = 'tb_comm_rules_zone_fk'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql := (
  ...
  WHERE table_schema = @db
    AND table_name = 'tb_commission_rules'
    AND constraint_name = 'tb_comm_rules_portefeuille_fk'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
-- =====================================================================
-- 233000 - snapshots transaction
-- =====================================================================

ALTER TABLE tb_transactions
  ADD COLUMN IF NOT EXISTS solde_compte_avant DECIMAL(18,2) NULL AFTER montant_commission_total,
  ADD COLUMN IF NOT EXISTS solde_compte_apres DECIMAL(18,2) NULL AFTER solde_compte_avant,
  ADD COLUMN IF NOT EXISTS montant_total_client DECIMAL(18,2) NULL AFTER solde_compte_apres,
  ADD COLUMN IF NOT EXISTS montant_net_client DECIMAL(18,2) NULL AFTER montant_total_client;

-- =====================================================================
-- 234000 - tables comptables
-- =====================================================================
...
-- =====================================================================
-- 235000 - seeds comptes + permissions compta
-- =====================================================================

INSERT INTO tb_plan_comptable (numero_compte, libelle, type_compte)
VALUES
...
('EBEN-ROL1','EBEN-PER49',NOW(),NOW()),
('EBEN-ROL1','EBEN-PER50',NOW(),NOW()),
('EBEN-ROL1','EBEN-PER51',NOW(),NOW()),
('EBEN-ROL1','EBEN-PER52',NOW(),NOW()),
('EBEN-ROL8','EBEN-PER49',NOW(),NOW()),
('EBEN-ROL8','EBEN-PER50',NOW(),NOW()),
('EBEN-ROL8','EBEN-PER51',NOW(),NOW()),
('EBEN-ROL8','EBEN-PER52',NOW(),NOW()),
('EBEN-ROL3','EBEN-PER49',NOW(),NOW()),
('EBEN-ROL3','EBEN-PER50',NOW(),NOW()),
('EBEN-ROL3','EBEN-PER52',NOW(),NOW()),
('EBEN-ROL5','EBEN-PER49',NOW(),NOW()),
('EBEN-ROL5','EBEN-PER50',NOW(),NOW()),
('EBEN-ROL5','EBEN-PER52',NOW(),NOW());

-- =====================================================================
-- 238000 - extension structure plan comptable OHADA
-- =====================================================================
...
UPDATE tb_plan_comptable
SET est_mouvementable = 0
WHERE CHAR_LENGTH(numero_compte) <= 2;
...
-- =====================================================================
-- 239000 - full OHADA chart (128 comptes) via UPSERT
-- =====================================================================
...
-- =====================================================================
-- 246000 - anti doublons et contraintes uniques (safe)
-- =====================================================================

-- Audit visuel
SELECT type_piece_identite, numero_piece_identite, COUNT(*) AS total
FROM tb_clients
WHERE numero_piece_identite IS NOT NULL AND numero_piece_identite <> ''
...
SET @dup_piece := (
  SELECT COUNT(*) FROM (
    SELECT type_piece_identite, numero_piece_identite
    FROM tb_clients
    WHERE numero_piece_identite IS NOT NULL AND numero_piece_identite <> ''
    GROUP BY type_piece_identite, numero_piece_identite
    HAVING COUNT(*) > 1
...
SET @idx_piece := (
  ...
  WHERE table_schema=@db AND table_name='tb_clients' AND index_name='uq_tb_clients_piece_type_num'
);
SET @sql := IF(@dup_piece=0 AND @idx_piece=0,
  'ALTER TABLE tb_clients ADD UNIQUE KEY uq_tb_clients_piece_type_num (type_piece_identite, numero_piece_identite)',
  'SELECT "Skip uq_tb_clients_piece_type_num (existing index or duplicates found)" AS info'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @idx_email := (
  SELECT COUNT(*) FROM information_schema.statistics
  WHERE table_schema=@db AND table_name='tb_clients' AND index_name='uq_tb_clients_email'
);
SET @sql := IF(@dup_email=0 AND @idx_email=0,
  'ALTER TABLE tb_clients ADD UNIQUE KEY uq_tb_clients_email (email)',
  'SELECT "Skip uq_tb_clients_email (existing index or duplicates found)" AS info'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
-- =====================================================================
-- Optional: mark these migrations as applied in Laravel migrations table
-- =====================================================================
...
('2026_03_13_210000_create_tb_commission_rules_and_transaction_commissions', @batch),
('2026_03_13_233000_add_accounting_snapshots_to_tb_transactions', @batch),
('2026_03_13_234000_create_tb_compta_journaux_and_ecritures', @batch),
('2026_03_13_235000_seed_ohada_accounts_and_comptabilite_permissions', @batch),
('2026_03_13_238000_extend_tb_plan_comptable_for_ohada_structure', @batch),
('2026_03_13_239000_seed_full_ohada_chart_accounts', @batch),
('2026_03_13_246000_add_unique_constraints_for_banking_integrity', @batch);

SELECT 'FULL SQL script done' AS status;
