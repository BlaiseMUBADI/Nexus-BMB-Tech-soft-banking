...
{{-- ── Titulaire + Photo ── --}}
<div style="display:table; width:100%; margin-bottom:14px; border:1px solid #c8e6c9;
            border-radius:5px; overflow:hidden;">
    @if($photoBase64)
    <div style="display:table-cell; width:80px; padding:8px; vertical-align:top;
                background:#f0fdf4; border-right:1px solid #c8e6c9;">
        <img src="{{ $photoBase64 }}" alt="Photo"
             style="width:68px; height:82px; object-fit:cover; border-radius:4px;
                    border:2px solid #1a7a4a;">
    </div>
    @endif
    <div style="display:table-cell; padding:8px 12px; vertical-align:top; background:#f0fdf4;">
        <div style="font-size:13px; font-weight:bold; color:#1a7a4a; margin-bottom:4px;">
    ...
    <div style="display:table-cell; width:180px; padding:8px 12px; vertical-align:top;
                background:#e6f4ea; border-left:1px solid #c8e6c9; text-align:center;">
        <div style="font-size:9px; color:#555; margin-bottom:3px; text-transform:uppercase;
                    letter-spacing:.5px;">Compte</div>
        <div style="font-size:10px; font-weight:bold; font-family:'Courier New', monospace;
                    color:#1a7a4a; margin-bottom:6px; word-break:break-all;">
            {{ $compte->code_compte }}
        </div>
        @php
        ...
        <div style="font-size:9px; color:#555;">Devise : <strong>{{ $compte->devise }}</strong></div>
    </div>
</div>
...
<table style="width:100%; border-collapse:collapse; margin-bottom:12px; font-size:9.5px;">
    <tr>
        <td style="background:#f8fafc; border:1px solid #dde; padding:5px 10px; color:#555;">
            Solde au {{ $dateDebut->isoFormat('D MMMM YYYY') }}
        </td>
        <td style="background:#f8fafc; border:1px solid #dde; padding:5px 10px;
...
<table style="width:100%; border-collapse:collapse; font-size:9px; margin-bottom:14px;">
    <thead>
        <tr style="background:#1a7a4a; color:#fff;">
            <th style="padding:5px 6px; text-align:left; width:55px;">Date</th>
            <th style="padding:5px 6px; text-align:left; width:70px;">Référence</th>
            <th style="padding:5px 6px; text-align:left;">Opération</th>
            <th style="padding:5px 6px; text-align:left; width:55px;">Observations</th>
            <th style="padding:5px 6px; text-align:right; width:68px;">Débit</th>
            <th style="padding:5px 6px; text-align:right; width:68px;">Crédit</th>
            <th style="padding:5px 6px; text-align:right; width:72px;">Solde</th>
        </tr>
    </thead>
    <tbody>
        ...
        <tr style="background:{{ $rowBg }}; border-bottom:1px solid #e0ede0;">
            ...
            <td style="padding:4px 6px; font-family:'Courier New',monospace; font-size:8px;">
                {{ $t->reference }}
            </td>
            <td style="padding:4px 6px;">
                {{ $typeLabels[$t->type] ?? $t->type }}
                @if($t->type === 'CHANGE' && $t->devise_dest)
                <br><span style="font-size:8px; color:#555;">→ {{ number_format($t->montant_dest,2,',','') }} {{ $t->devise_dest }}</span>
                @endif
            </td>
            <td style="padding:4px 6px; font-size:8px; color:#666;">
                {{ \Illuminate\Support\Str::limit($t->observations ?? '', 30) }}
            </td>
            <td style="padding:4px 6px; text-align:right; color:{{ $isDebit ? '#c0392b' : '' }}; font-weight:{{ $isDebit ? 'bold' : 'normal' }};">
                {{ $isDebit ? number_format($montant, 2, ',', ' ') : '' }}
            </td>
            <td style="padding:4px 6px; text-align:right; color:{{ $isCredit ? '#1a7a4a' : '' }}; font-weight:{{ $isCredit ? 'bold' : 'normal' }};">
    ...
    <tfoot>
        <tr style="background:#e6f4ea; font-weight:bold; border-top:2px solid #1a7a4a;">
            <td colspan="4" style="padding:5px 6px; text-align:right; font-size:9.5px;">TOTAUX DE LA PÉRIODE</td>
            <td style="padding:5px 6px; text-align:right; color:#c0392b;">
                {{ number_format($totalDebit, 2, ',', ' ') }}
            </td>
            <td style="padding:5px 6px; text-align:right; color:#1a7a4a;">
...
@else
<div style="text-align:center; padding:20px; border:1px dashed #c8e6c9; border-radius:4px;
            color:#555; margin-bottom:14px; font-size:10px;">
...
<table style="width:100%; border-collapse:collapse; font-size:10px; margin-bottom:20px;">
    <tr>
        <td style="background:#1a7a4a; color:#fff; padding:6px 12px; border-radius:4px 0 0 4px; font-weight:bold;">
            SOLDE AU {{ $dateFin->isoFormat('D MMMM YYYY') }}
        </td>
        <td style="background:#e6f4ea; border:1px solid #1a7a4a; padding:6px 12px;
...
{{-- ── Notice légale ── --}}
<div style="font-size:8px; color:#888; border-top:1px solid #dde; padding-top:8px; text-align:center;">
