    ...
    <div class="container-fluid">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div id="clients-table-toolbar"
                    class="d-flex flex-wrap align-items-center justify-content-between mb-3 p-2 rounded shadow-sm"
                    style="background: #232a32; border: 1px solid #444; min-height: 56px;">
                    <div id="clients-table-buttons" class="mb-2 mb-md-0 "></div>
                    <div id="clients-table-search" class="datatable-search ms-md-3 "></div>
                </div>

                <div class="table-responsive">
                    <table id="clients-table" class="table table-bordered table-striped datatable"
                        data-buttons-container="#clients-table-buttons">
                        <thead>
                            <tr>
                                <th>N°</th>
                                <th>Matricule</th>
                        ...
                        <tbody>
                            @foreach($clients as $loopIndex => $client)
                                <tr>
                                    ...
                                    <td>
                                        @if($client->photo)
                                            <img src="{{ route('clients.photo', basename($client->photo)) }}?v={{ time() }}"
                                                alt="Photo" class="client-photo-thumb"
                                                    ...
                                                    <div class="modal-content bg-dark">
                                                        <div class="modal-header border-0">
                                                            <h5 class="modal-title text-white"
                                                                id="photoModalLabel-{{ $client->matricule }}">Photo du client </h5>
                                                        ...
                                                        <div class="modal-body text-center">
                                                            <img src="{{ route('clients.photo', basename($client->photo)) }}"
                                                                alt="Photo du client"
                                                                class="img-fluid rounded shadow photo-modal-img"
                                    ...
                                    <td>
                                        <a href="{{ url('/clients/' . $client->matricule) }}" class="btn btn-sm btn-info"
                                            title="Voir">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ url('/clients/' . $client->matricule . '/edit') }}"
                                            class="btn btn-sm btn-warning" title="Modifier">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ url('/clients/' . $client->matricule) }}" method="POST"
                                            class="d-inline delete-client-form"
                                            data-client-matricule="{{ $client->matricule }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-sm btn-danger btn-delete-client"
                                                data-matricule="{{ $client->matricule }}" title="Supprimer">
